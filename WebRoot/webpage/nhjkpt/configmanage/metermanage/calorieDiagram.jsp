 ...
 <head>
  ...
  var jg= new jsGraphics("meter");
	$(document).ready(function() {
	  $.ajax({
			type : "POST",
			url : "meterController.do?queryCalorieMeters&roomid=${roomid}",
			success : function(jsondata) {
				var dx=20;
				var dy=20;
				jg.drawLine(dx+harf,dy,dx+harf,dy+addx);
				jg.drawImage("plug-in/jsgraphics/images/calorie/All_U.png", dx,dy+100,25,100); 
				jg.setColor("#55bb55");
				jg.paint();
	...
	function drawcirfirst(data,parentid,x,y){
		...
		for(var i=0;i<data.length;i++){
			if(data[i].parentid==parentid){
				...
				if(meterId && meterId.indexOf("8")==0 )
				{
					jg.drawString(""+data[i].daysum+"gj",ax+addx/2+105,dy+addy+5);
				}else{
	...
	function drawcir(data,parentid,x,y){
		...
		for(var i=0;i<data.length;i++){
			if(data[i].parentid==parentid){
				...
				if(meterId && meterId.indexOf("8")==0 )
				{
					jg.drawString(""+data[i].daysum+"gj",dx+addx+105,dy+addy+5);
				}else{
