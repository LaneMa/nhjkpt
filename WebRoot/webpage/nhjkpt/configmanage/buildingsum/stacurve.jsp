...
<script type="text/javascript">
	...
	$(function(){
		$(document).ready(function() {
			url="buildingSumController.do?queryBroswerBar&buildingid=${buildingid}";
			//loadData(url);
			loadDataByAjax(url,"buildingSumline","建筑物用能总量统计表","日期","用能","折线图");
		});
	});
	...
	function doSubmitSum(){
		url="buildingSumController.do?queryBroswerBar&buildingid=${buildingid}&funcid="+$("#buildingfuncid").val()+"&type="+$("#type").val()+"&startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val();
		//loadData(url);
		loadDataByAjax(url,"buildingSumline","建筑物用能总量统计表","日期","用能","折线图");
	}
	function doExportSum(){
		window.location.href="buildingSumController.do?exportExcel&buildingid=${buildingid}&funcid="+$("#buildingfuncid").val()+"&type="+$("#type").val()+"&startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val();
	}
</script>
</head>
...
<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable" style="overflow-y: hidden" scroll="no">
	<tr>
		...
		<td class="value">
			<select name="funcid" id="buildingfuncid">
				<option value="">--请选择--</option>
				<c:forEach items="${funclist}" var="func">
					<option value="${func.funcid}"
						<c:if test="${func.funcid==schoolSumPage.funcid }">selected="selected"</c:if>>
						${func.showtext}
					</option>
				</c:forEach>
			</select>
...
<div id="buildingSumline" style="width:780px; height:350px"></div>
</html>