...
<html>
 <head>
  <title>学校分类用能计算用表</title>
  <t:base type="jquery,easyui,tools,DatePicker"></t:base>
 </head>
 <body style="overflow-y: hidden" scroll="no">
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="schoolItemizeController.do?save">
			<input id="id" name="id" type="hidden" value="${schoolItemizePage.id }">
			<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
				<tr>
					...
					<td class="value">
						<input name="itemizeid" id="itemizeid" type="hidden" value="${schoolItemizePage.itemizeid}"> 
				...
				<tr>
					...
					<td class="value">
						<input name="meterid" id="meterid" type="hidden" value="${schoolItemizePage.meterid}"> 
					...
					<td class="value">
						...
						<c:forEach items="${funclist}" var="func">
							<option value="${func.funcid}"
								<c:if test="${fn:trim(func.funcid)==schoolItemizePage.funcid }">selected="selected"</c:if>>
								${func.funcname}
							</option>
						</c:forEach>
						</select>
				...
				<tr>
					...
					<td class="value">
						<input class="inputxt" id="factor" name="factor" ignore="ignore"
							   value="${schoolItemizePage.factor}" datatype="n">
						<span class="Validform_checktip"></span>
					</td>
				</tr>
			</table>
