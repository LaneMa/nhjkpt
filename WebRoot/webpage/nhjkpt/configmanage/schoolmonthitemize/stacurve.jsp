...
<script type="text/javascript">
	$(function() {
		$(document).ready(function() {
			var url="schoolMonthItemizeController.do?queryBroswerBar";
			//loadDataItemize(url);
			loadDataByAjax(url,"itemizeline","学校用电分类统计表","日期","用能(千瓦时)","折线图");
		});
	});
	function loadDataItemize(url){
		var chart;
		$.ajax({
			type : "POST",
			url : url,
	...
	function doSubmitItemize(){
		var url="schoolMonthItemizeController.do?queryBroswerBar&itemizeid="+$("#schoolmonthitemizeidhidden").val()+"&type="+$("#type").val()+"&startDate="+$("#startDate").val()+"&endDate="+$("#endDate").val();
		//loadDataItemize(url);
		loadDataByAjax(url,"itemizeline","学校用电分类统计表","日期","用能(千瓦时)","折线图");
	}
</script>
...
<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable" style="overflow-y: hidden" scroll="no">
	<tr>
		...
		<td class="value">
			<t:comboTree url="itemizeController.do?itemizeTree" name="itemizeid" id="schoolmonthitemizeid"></t:comboTree> 
...
<div id="itemizeline" style="width:100%; height:80%"></div>
</html>