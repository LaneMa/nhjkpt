package org.framework.tag.core.easyui;
...
import org.framework.tag.vo.easyui.ColumnValue;
import org.framework.tag.vo.easyui.DateGridColumn;
import org.framework.tag.vo.easyui.DateGridUrl;
import org.framework.tag.vo.easyui.OptTypeDirection;
...
public class DataGridTag extends TagSupport {
	...
	private String onClickCell;// 单元格单击事件调用方法
	 ...
	 * 设置工具条
	 */
	public void setToolbar(String url, String title, String icon, String exp,String onclick, String funname,String operationCode) {
		DateGridUrl dateGridUrl = new DateGridUrl();
		dateGridUrl.setTitle(title);
		dateGridUrl.setUrl(url);
	 ...
	 * @param value
	 */
	public void setColumn(String title, String field, Integer width, String rowspan, String colspan, String align, boolean sortable, boolean checkbox, String formatter, boolean hidden, String replace, String treefield, boolean image, boolean query, String url, String funname, String arg, String queryMode, String dictionary,boolean frozenColumn,String editor) {
		...
		dateGridColumn.setEditor(editor);
	...
	public StringBuffer end() {
		...
		if (StringUtil.isNotEmpty(onClickCell)) {
			sb.append("onClickCell:function(index, field){" + onClickCell + "(index, field);},");
		}
...
//				}
//			}
			//update-begin--Author: sun  Date:20130506 for：解决查询条件取值非input类型是，取值为空的bug  注：20130507赵俊夫进一步修复
			sb.append("$(\'#" + name + "tb\').find('*').each(function(){queryParams[$(this).attr('name')]=$(this).val();});");
			//update-end--Author: sun  Date:20130506 for：解决查询条件取值非input类型是，取值为空的bug
			...
			//update-begin--Author: sun  Date:20130506 for：解决查询条件重置非input类型时，失效的问题
		...
		if(toolBarList.size()>0)
		{
			...
			//update-begin--Author:zhaojunfu  Date:20130419 for：生成组合查询，当没有查询字段时不生成查询框
			...
			//update-begin--Author:zhaojunfu  Date:20130526 for：131 IE兼容性问题一单查询的查询按钮无效
			sb.append("<div style=\"height:30px;\" class=\"datagrid-toolbar\">");
			//update-end--Author:zhaojunfu  Date:20130526 for：131 IE兼容性问题一单查询的查询按钮无效
...
//				sb.append("</div>");
//				sb.append("</div>");
			//update-end--Author:zhaojunfu  Date:20130419 for：生成组合查询，当没有查询字段时不生成查询框
			sb.append("</div>");
		}
		return sb;
	}
	...
	protected void getField(StringBuffer sb,int frozen) {
		...
		for (DateGridColumn column : columnList) {
			...
			// 自定义编辑信息
			if (column.getEditor() != null && !column.getEditor().equals("")) {
