...
import org.framework.core.annotation.JeecgEntityTitle;
...
import org.framework.core.common.hibernate.qbc.DetachedCriteriaUtil;
import org.framework.core.common.hibernate.qbc.HqlQuery;
import org.framework.core.common.hibernate.qbc.PageList;
import org.framework.core.common.hibernate.qbc.PagerUtil;
...
import org.framework.core.util.ToEntityUtil;
...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	...
	public DataGridReturn getDataGridReturn(final CriteriaQuery cq, final boolean isOffset) {
		...
		// 先把Projection和OrderBy条件取出来,清空两者来执行Count操作
		Projection projection = impl.getProjection();
		//System.out.println("dddd====:"+Projections.rowCount());
		final int allCounts = ((Long) criteria.setProjection(Projections.rowCount()).uniqueResult()).intValue();
		criteria.setProjection(projection);
		if (projection == null) {
			criteria.setResultTransformer(CriteriaSpecification.ROOT_ENTITY);
