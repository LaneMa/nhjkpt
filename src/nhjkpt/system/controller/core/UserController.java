...
import org.framework.core.common.model.json.ComboBox;
...
import org.framework.core.util.PasswordUtil;
import org.framework.core.util.ResourceUtil;
import org.framework.core.util.SetListSort;
...
import org.framework.tag.vo.datatable.DataTables;
...
public class UserController {
	...
	public String changetime(HttpServletRequest request) {
		
		request.setAttribute("nowdate", new Date());
		return "system/user/changetime";
	}
	...
	@RequestMapping(params = "savetime")
	@ResponseBody
	public AjaxJson savetime(java.util.Date createdate,HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		try{
		Runtime.getRuntime().exec("cmd /c date 2015-06-28"); 
		
		Runtime.getRuntime().exec("cmd /c time 12:00:00");  
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		//查询条件组装器
		org.framework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, user);
